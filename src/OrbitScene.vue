...
const props=defineProps({progress:{type:Array,default:()=>[0,0,0]},active:{type:Number,default:-1},revision:{type:Number,default:0},immersive:{type:Boolean,default:false},suspended:{type:Boolean,default:false}});
...
function setMotion(){scene?.setPaused(paused.value)}
function onPreference(e){paused.value=e.matches;setMotion()}
function fallback(){ready.value=false;failed.value=true}
onMounted(async()=>{
  media=matchMedia('(prefers-reduced-motion: reduce)');paused.value=media.matches;media.addEventListener('change',onPreference);
  try {const {createOrbit}=await import('./orbit');if(disposed)return;scene=createOrbit(host.value,{reduced:paused.value,immersive:props.immersive,onModelFailure:()=>modelError.value=true,onSelect:index=>emit('select',index),onFailure:fallback});scene.setProgress(props.progress);scene.setActive(props.active);scene.setSuspended(props.suspended);ready.value=true;}catch{fallback()}
});
